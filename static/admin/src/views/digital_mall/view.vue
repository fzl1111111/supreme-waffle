		...
		<el-row class="row_ce"> 
            ...
            <el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="商品编号" prop="product_number">
							  ...
							  v-if="user_group === '管理员' || (form['digital_mall_id'] && $check_field('set','product_number')) || (!form['digital_mall_id'] && $check_field('add','product_number'))" :disabled="disabledObj['product_number_isDisabled']"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/digital_mall/view','set') || $check_action('/digital_mall/view','add') || $check_option('/digital_mall/table','examine')">
	...
	export default {
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/digital_mall/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/digital_mall/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/digital_mall/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/digital_mall/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/digital_mall/view','get');
            ...
            submit_after(){
                ...
                if (this.form[this.field]){
                    ...
                    this.$get("~/api/digital_mall/get_obj?",query_form,(res)=>{
            ...
            submit_goods(source_id,add_flag){
				let customize_field = []
							customize_field.push({"field_name":"商品编号","field_value":this.form.product_number});
			                this.goods_form = {
                    goods_id: 0,
                ...
                if (add_flag==='set'){
                    ...
                    // 提交事件
                    this.$post("~/api/goods/set?source_table=digital_mall&source_field=" + this.field + "&source_id=" + source_id,this.goods_form,(res)=>{
                        console.log("提交结果：" ,res)
                        if(res.result){
                            this.$toast("修改成功!" ,"success");
							this.$router.push("/digital_mall/table");
                        }else if(res.error){
                            console.error(res.error);
                            this.$toast(res.error.message,"error");
                        }
