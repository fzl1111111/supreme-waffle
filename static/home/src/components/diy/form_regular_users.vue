<template>
	<div class="diy_form">
			...
			<b-form-select id="input-${idx}" v-model="form['user_gender']" :options="getDropList(list_user_gender)"></b-form-select>
				...
				<b-form-group v-if="$check_register_field('add','user_phone_number','/regular_users/view')" id="input-group-${idx}" label="用户电话" label-for="input-${idx}">
			<b-form-input id="input-${idx}" v-model="form['user_phone_number']" type="text" placeholder="用户电话" trim></b-form-input>
		</b-form-group>
		</div>
</template>
